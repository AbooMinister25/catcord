...
async def token(userinfo: UserCreateBody):
    ...
    await tasks.new_user(conn, str(actions.gensnowflake()), tokenhash, userinfo.username, passhash)
    await conn.close()
    return {"token": token}

@app.post("/new_server")
async def new_server(response: Response, serverinfo: NewServerBody, Auth: str = Header(None)):
    if Auth is None:
    ...
    if userdata is None:
        response.status_code = 403
        return {"error": "Token supplied is invalid. Please correct your token or get one by sending a post request to /token ."}
...
async def new_message(response: Response, messageinfo: NewMessageBody, Auth: str = Header(None)):
    if Auth is None:
        response.status_code = 403
        return {"error": "No token supplied. Please submit a token."}
    ...
    await tasks.new_message(conn, messageid, time.time_ns(), userdata[0], messageinfo.server_id, messageinfo.message_content)
    return {"message_id": messageid}

@app.get("/get_messages")
async def get_messages(response: Response, server_id: str = None, Auth: str = Header(None)):
    if Auth is None:
        return {"error": "No token supplied. Please submit a token."}
    conn = await tasks.get_db()
    tokenhash = actions.gentokenhash(Auth)
    userdata = await conn.fetchrow(f"SELECT * FROM USERS WHERE TOKEN='{tokenhash}'")
    if userdata is None:
        return {"error": "Token supplied is invalid. Pleasse correct your token or get one by sending a post request to /tokens ."}
    ...
    for element in messages:
        ...
        sender_name = await conn.fetchrow(f"SELECT USERNAME FROM USERS WHERE ID='{element[2]}'")
